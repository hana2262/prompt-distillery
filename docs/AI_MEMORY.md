...
| 存档版本 | v1.0.0 |
| 首次发布日期 | 2026-02-27 |
| 文档覆盖范围 | 仅覆盖 v1.0.0 功能，不含 v1.1.0「数据管理/备份」等后续版本变更 |
| 当前最新版本 | v1.1.0（详情参见 CHANGELOG / 发布说明） |
...
| `data:export` | 导出数据到 JSON 文件 |
| `data:import` | 从 JSON 文件导入数据 |
| `data:backup` | 手动备份数据 |
| `data:auto-backup` | 自动备份数据 |
| `data:get-backup-path` | 获取备份目录路径 |
| `data:open-backup-folder` | 打开备份目录 |
...
**注意**：数据相关 IPC 需要渲染进程传入数据 payload，因为主进程无法直接访问 localStorage。
...
5. **主进程 localStorage 错误**：改为由渲染进程传入数据 payload
